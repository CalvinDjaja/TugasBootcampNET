    ...
    public class CourseEF : ICourse
    {
        ...
        public IEnumerable<Course> AllCourseWithChosenStudent(int StudentID)
        {
            var course = _context.Courses
                .FromSqlInterpolated($"exec dbo.AllCourseWithChosenStudent {StudentID}").ToList();
            return course;
        }
        ...
        public IEnumerable<Course> GetByName(string Title)
        {
            var courses = _context.Courses.Where(c => c.Title.Contains(Title)).OrderBy(c => c.Title);
            return courses;
        }
